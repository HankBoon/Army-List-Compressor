const tauEmpire = {
  name: "T'au Empire",
  detachment: [
    "Kauyon",
    "Kroot Hunting Pack",
    "Mont'ka",
    "Retaliation Cadre"
  ],
  units: [
    {
      name: "Cadre Fireblade",
      points: 140,
      warlord: true,
      weapons: [
        { name: "Close combat weapon", display: "", alias: "" },
        { name: "Fireblade pulse rifle", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" }
      ],
      enhancements: [
        "Exemplar of the Kauyon",
        "Solid-image Projection Unit",
        "Precision of the Patient Hunter",
        "Through Unity, Devastation"
      ]
    },
    {
      name: "Commander Farsight",
      points: 105,
      warlord: true,
      weapons: [
        { name: "Dawn Blade", display: "", alias: "" },
        { name: "High-intensity plasma rifle", display: "", alias: "" }
      ]
    },
    {
      name: "Commander Shadowsun",
      points: 100,
      warlord: true,
      weapons: [
        { name: "Advanced Guardian Drone", display: "true", alias: "Drone" },
        { name: "Battlesuit fists", display: "", alias: "" },
        { name: "Command-link Drone (Aura)", display: "", alias: "" },
        { name: "Flechette launcher", display: "", alias: "" },
        { name: "High-energy fusion blaster", display: "", alias: "" },
        { name: "Light missile pod", display: "", alias: "" },
        { name: "Pulse pistol", display: "", alias: "" }
      ]
    },
    {
      name: "Commander in Coldstar Battlesuit",
      points: 185,
      warlord: true,
      weapons: [
        { name: "Airbursting fragmentation projector", display: "", alias: "" },
        { name: "Battlesuit Support System", display: "", alias: "" },
        { name: "Battlesuit fists", display: "", alias: "" },
        { name: "Burst cannon", display: "", alias: "" },
        { name: "Cyclic ion blaster", display: "", alias: "" },
        { name: "Fusion blaster", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "High-output burst cannon", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Missile pod", display: "", alias: "" },
        { name: "Plasma rifle", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" },
        { name: "Shield Generator", display: "", alias: "" },
        { name: "T'au flamer", display: "", alias: "" },
        { name: "Weapon Support System", display: "", alias: "" }
      ],
      enhancements: [
        "Exemplar of the Kauyon",
        "Solid-image Projection Unit",
        "Precision of the Patient Hunter",
        "Through Unity, Devastation"
      ]
    },
    {
      name: "Commander in Enforcer Battlesuit",
      points: 170,
      warlord: true,
      weapons: [
        { name: "Airbursting fragmentation projector", display: "", alias: "" },
        { name: "Battlesuit Support System", display: "", alias: "" },
        { name: "Battlesuit fists", display: "", alias: "" },
        { name: "Burst cannon", display: "", alias: "" },
        { name: "Cyclic ion blaster", display: "", alias: "" },
        { name: "Fusion blaster", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Missile pod", display: "", alias: "" },
        { name: "Plasma rifle", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" },
        { name: "Shield Generator", display: "", alias: "" },
        { name: "T'au flamer", display: "", alias: "" },
        { name: "Weapon Support System", display: "", alias: "" }
      ],
      enhancements: [
        "Exemplar of the Kauyon",
        "Solid-image Projection Unit",
        "Precision of the Patient Hunter",
        "Through Unity, Devastation"
      ]
    },
    {
      name: "Darkstrider",
      points: 60,
      warlord: true,
      weapons: [
        { name: "Close combat weapon", display: "", alias: "" },
        { name: "Shade", display: "", alias: "" }
      ]
    },
    {
      name: "Ethereal",
      points: 140,
      warlord: true,
      weapons: [
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Honour stave", display: "", alias: "" },
        { name: "Hover Drone", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" }
      ],
      enhancements: [
        "Exemplar of the Kauyon",
        "Solid-image Projection Unit",
        "Precision of the Patient Hunter",
        "Through Unity, Devastation"
      ]
    },
    {
      name: "Firesight Team",
      points: 160,
      warlord: true,
      weapons: [
        { name: "Close combat weapons", display: "", alias: "" },
        { name: "Longshot pulse rifles", display: "", alias: "" },
        { name: "Pulse pistol", display: "", alias: "" }
      ],
      enhancements: [
        "Exemplar of the Kauyon",
        "Solid-image Projection Unit",
        "Precision of the Patient Hunter",
        "Through Unity, Devastation"
      ]
    },
    {
      name: "Kroot Flesh Shaper",
      points: 100,
      warlord: true,
      weapons: [
        { name: "Kroot scattergun", display: "", alias: "" },
        { name: "Twin ritualistic blades", display: "", alias: "" }
      ],
      enhancements: [
        "Solid-image Projection Unit",
        "Precision of the Patient Hunter"
      ]
    },
    {
      name: "Kroot Lone-spear",
      points: 180,
      warlord: true,
      weapons: [
        { name: "Blast javelin", display: "", alias: "" },
        { name: "Close combat weapon", display: "", alias: "" },
        { name: "Hunting javelin", display: "", alias: "" },
        { name: "Kalamandra's bite", display: "", alias: "" },
        { name: "Kroot long gun", display: "", alias: "" }
      ],
      enhancements: [
        "Exemplar of the Kauyon",
        "Solid-image Projection Unit",
        "Precision of the Patient Hunter",
        "Through Unity, Devastation"
      ]
    },
    {
      name: "Kroot Trail Shaper",
      points: 110,
      warlord: true,
      weapons: [
        { name: "Kroot rifle", display: "", alias: "" },
        { name: "Shaper's blade", display: "", alias: "" }
      ],
      enhancements: [
        "Solid-image Projection Unit",
        "Precision of the Patient Hunter"
      ]
    },
    {
      name: "Kroot War Shaper",
      points: 105,
      warlord: true,
      weapons: [
        { name: "Bladestave and prey-hook", display: "", alias: "" },
        { name: "Dart-bow and tri-blade", display: "", alias: "" },
        { name: "Kroot pistol", display: "", alias: "" },
        { name: "Shaper's blade", display: "", alias: "" }
      ],
      enhancements: [
        "Solid-image Projection Unit",
        "Precision of the Patient Hunter"
      ]
    },
    {
      name: "Breacher Team",
      points: 100,
      weapons: [
        { name: "Close combat weapon", display: "", alias: "" },
        { name: "Pulse blaster", display: "", alias: "" },
        { name: "Pulse pistol", display: "", alias: "" },
        { name: "Guardian Drone", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" },
        { name: "Support turret", display: "", alias: "" }
      ]
    },
    {
      name: "Strike Team",
      points: 75,
      weapons: [
        { name: "Close combat weapon", display: "", alias: "" },
        { name: "Pulse pistol", display: "", alias: "" },
        { name: "Pulse rifle", display: "", alias: "" },
        { name: "Guardian Drone", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" },
        { name: "Support turret", display: "", alias: "" }
      ]
    },
    {
      name: "Devilfish",
      points: 85,
      weapons: [
        { name: "Accelerator burst cannon", display: "", alias: "" },
        { name: "Armoured hull", display: "", alias: "" },
        { name: "Seeker missile", display: "", alias: "" },
        { name: "Smart missile system", display: "", alias: "" },
        { name: "Twin pulse carbine", display: "", alias: "" }
      ]
    },
    {
      name: "AX-1-0 Tiger Shark",
      points: 315,
      weapons: [
        { name: "Armoured hull", display: "", alias: "" },
        { name: "Burst cannon", display: "", alias: "" },
        { name: "Cyclic ion blaster", display: "", alias: "" },
        { name: "Missile pod", display: "", alias: "" },
        { name: "Seeker missile", display: "", alias: "" },
        { name: "Twin heavy rail cannon", display: "", alias: "" }
      ]
    },
    {
      name: "Broadside Battlesuits",
      points: 90,
      weapons: [
        { name: "Crushing bulk", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Heavy rail rifle", display: "", alias: "" },
        { name: "High-yield missile pods", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Missile Drone", display: "", alias: "" },
        { name: "Seeker missile", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" },
        { name: "Twin plasma rifle", display: "", alias: "" },
        { name: "Twin smart missile system", display: "", alias: "" },
        { name: "Weapon Support System", display: "", alias: "" }
      ]
    },
    {
      name: "Crisis Fireknife Battlesuits",
      points: 130,
      weapons: [
        { name: "Battlesuit fists", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Missile pod", display: "", alias: "" },
        { name: "Plasma rifle", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" }
      ]
    },
    {
      name: "Crisis Starscythe Battlesuits",
      points: 110,
      weapons: [
        { name: "Battlesuit fists", display: "", alias: "" },
        { name: "Burst cannon", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" },
        { name: "T'au flamer", display: "", alias: "" }
      ]
    },
    {
      name: "Crisis Sunforge Battlesuits",
      points: 150,
      weapons: [
        { name: "Battlesuit fists", display: "", alias: "" },
        { name: "Fusion blaster", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" }
      ]
    },
    {
      name: "Ghostkeel Battlesuit",
      points: 160,
      weapons: [
        { name: "Battlesuit Support System", display: "", alias: "" },
        { name: "Cyclic ion raker", display: "", alias: "" },
        { name: "Fusion collider", display: "", alias: "" },
        { name: "Ghostkeel fists", display: "", alias: "" },
        { name: "Twin T'au flamer", display: "", alias: "" },
        { name: "Twin burst cannon", display: "", alias: "" },
        { name: "Twin fusion blaster", display: "", alias: "" }
      ]
    },
    {
      name: "Hammerhead Gunship",
      points: 145,
      weapons: [
        { name: "Accelerator burst cannon", display: "", alias: "" },
        { name: "Armoured hull", display: "", alias: "" },
        { name: "Ion cannon", display: "", alias: "" },
        { name: "Railgun", display: "", alias: "" },
        { name: "Seeker missile", display: "", alias: "" },
        { name: "Twin pulse carbine", display: "", alias: "" },
        { name: "Twin smart missile system", display: "", alias: "" }
      ]
    },
    {
      name: "Kroot Carnivores",
      points: 75,
      weapons: [
        { name: "Close combat weapon", display: "", alias: "" },
        { name: "Kroot carbine", display: "", alias: "" },
        { name: "Kroot pistol", display: "", alias: "" },
        { name: "Kroot rifle", display: "", alias: "" },
        { name: "Tanglebomb launcher", display: "", alias: "" }
      ]
    },
    {
      name: "Kroot Farstalkers",
      points: 85,
      weapons: [
        { name: "Farstalker firearm", display: "", alias: "" },
        { name: "Kroot pistol", display: "", alias: "" },
        { name: "Ritual blade", display: "", alias: "" },
        { name: "T'au-tech rifle", display: "", alias: "" },
        { name: "Close combat weapon", display: "", alias: "" },
        { name: "Ripping fangs", display: "", alias: "" },
        { name: "Dvorgite skinner", display: "", alias: "" },
        { name: "Londaxi tribalest", display: "", alias: "" },
        { name: "Pech'ra", display: "", alias: "" }
      ]
    },
    {
      name: "Kroot Hounds",
      points: 40,
      maxCount: 10,
      weapons: [
        { name: "Ripping fangs", display: "", alias: "" }
      ]
    },
    {
      name: "Krootox Rampagers",
      points: 110,
      maxCount: 6,
      weapons: [
        { name: "Hunting blades", display: "", alias: "" },
        { name: "Kroot pistol and hunting javelins", display: "", alias: "" },
        { name: "Rampager fists", display: "", alias: "" }
      ]
    },
    {
      name: "Krootox Riders",
      points: 35,
      maxCount: 3,
      weapons: [
        { name: "Close combat weapon", display: "", alias: "" },
        { name: "Krootox fists", display: "", alias: "" },
        { name: "Repeater cannon", display: "", alias: "" },
        { name: "Tanglecannon", display: "", alias: "" }
      ]
    },
    {
      name: "Manta",
      points: 2100,
      weapons: [
        { name: "Armoured hull", display: "", alias: "" },
        { name: "Heavy rail cannon", display: "", alias: "" },
        { name: "Ion cannon", display: "", alias: "" },
        { name: "Long-barrelled burst cannon array", display: "", alias: "" },
        { name: "Missile pod", display: "", alias: "" },
        { name: "Seeker missile", display: "", alias: "" }
      ]
    },
    {
      name: "Pathfinder Team",
      points: 90,
      weapons: [
        { name: "Close combat weapon", display: "", alias: "" },
        { name: "Grav-inhibitor Drone", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Pulse Accelerator Drone", display: "", alias: "" },
        { name: "Pulse carbine", display: "", alias: "" },
        { name: "Pulse pistol", display: "", alias: "" },
        { name: "Recon Drone", display: "", alias: "" },
        { name: "Semi-automatic grenade launcher", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" },
        { name: "Ion rifle", display: "", alias: "" },
        { name: "Rail rifle", display: "", alias: "" }
      ]
    },
    {
      name: "Piranhas",
      points: 55,
      maxCount: 3,
      weapons: [
        { name: "Armoured hull", display: "", alias: "" },
        { name: "Piranha burst cannon", display: "", alias: "" },
        { name: "Piranha fusion blaster", display: "", alias: "" },
        { name: "Seeker missile", display: "", alias: "" },
        { name: "Twin pulse carbine", display: "", alias: "" }
      ]
    },
    {
      name: "Razorshark Strike Fighter",
      points: 170,
      weapons: [
        { name: "Accelerator burst cannon", display: "", alias: "" },
        { name: "Armoured hull", display: "", alias: "" },
        { name: "Missile pod", display: "", alias: "" },
        { name: "Quad ion turret", display: "", alias: "" },
        { name: "Seeker missile", display: "", alias: "" }
      ]
    },
    {
      name: "Riptide Battlesuit",
      points: 180,
      weapons: [
        { name: "Heavy burst cannon", display: "", alias: "" },
        { name: "Ion accelerator", display: "", alias: "" },
        { name: "Missile Drone", display: "", alias: "" },
        { name: "Riptide fists", display: "", alias: "" },
        { name: "Twin fusion blaster", display: "", alias: "" },
        { name: "Twin plasma rifle", display: "", alias: "" },
        { name: "Twin smart missile system", display: "", alias: "" }
      ]
    },
    {
      name: "Sky Ray Gunship",
      points: 140,
      weapons: [
        { name: "Accelerator burst cannon", display: "", alias: "" },
        { name: "Armoured hull", display: "", alias: "" },
        { name: "Seeker missile rack", display: "", alias: "" },
        { name: "Smart missile system", display: "", alias: "" },
        { name: "Twin pulse carbine", display: "", alias: "" }
      ]
    },
    {
      name: "Stealth Battlesuits",
      points: 60,
      weapons: [
        { name: "Battlesuit Support System", display: "", alias: "" },
        { name: "Battlesuit fists", display: "", alias: "" },
        { name: "Burst cannon", display: "", alias: "" },
        { name: "Fusion blaster", display: "", alias: "" },
        { name: "Gun Drone", display: "", alias: "" },
        { name: "Homing Beacon", display: "", alias: "" },
        { name: "Marker Drone", display: "", alias: "" },
        { name: "Shield Drone", display: "", alias: "" }
      ]
    },
    {
      name: "Stormsurge",
      points: 400,
      weapons: [
        { name: "Cluster rocket system", display: "", alias: "" },
        { name: "Destroyer missiles", display: "", alias: "" },
        { name: "Pulse blast cannon", display: "", alias: "" },
        { name: "Pulse driver cannon", display: "", alias: "" },
        { name: "Thunderous footfalls", display: "", alias: "" },
        { name: "Twin T'au flamer", display: "", alias: "" },
        { name: "Twin airbursting fragmentation projector", display: "", alias: "" },
        { name: "Twin burst cannon", display: "", alias: "" },
        { name: "Twin smart missile system", display: "", alias: "" }
      ]
    },
    {
      name: "Sun Shark Bomber",
      points: 160,
      weapons: [
        { name: "Armoured hull", display: "", alias: "" },
        { name: "Missile pod", display: "", alias: "" },
        { name: "Seeker missile", display: "", alias: "" },
        { name: "Twin ion rifle", display: "", alias: "" },
        { name: "Twin missile pod", display: "", alias: "" }
      ]
    },
    {
      name: "Ta'unar Supremacy Armour",
      points: 790,
      weapons: [
        { name: "Burst cannon", display: "", alias: "" },
        { name: "Crushing feet", display: "", alias: "" },
        { name: "Fragmentation cluster shell launcher", display: "", alias: "" },
        { name: "Fusion eradicator", display: "", alias: "" },
        { name: "Heavy rail cannon array", display: "", alias: "" },
        { name: "Nexus missile launcher", display: "", alias: "" },
        { name: "Pulse ordnance driver", display: "", alias: "" },
        { name: "Smart missile system", display: "", alias: "" },
        { name: "Tri-axis ion cannon", display: "", alias: "" }
      ]
    },
    {
      name: "Tidewall Droneport",
      points: 85,
      weapons: [
        { name: "Drone defenders", display: "", alias: "" }
      ]
    },
    {
      name: "Tidewall Gunrig",
      points: 90,
      weapons: [
        { name: "Supremacy railgun", display: "", alias: "" }
      ]
    },
    {
      name: "Tidewall Shieldline",
      points: 105,
      weapons: [
        { name: "Tidewall Defence Platform", display: "", alias: "" },
        { name: "Tidewall Shieldline", display: "", alias: "" }
      ]
    },
    {
      name: "Tiger Shark",
      points: 275,
      weapons: [
        { name: "Armoured hull", display: "", alias: "" },
        { name: "Burst cannon", display: "", alias: "" },
        { name: "Cyclic ion blaster", display: "", alias: "" },
        { name: "Ion cannon", display: "", alias: "" },
        { name: "Missile pod", display: "", alias: "" },
        { name: "Seeker missile", display: "", alias: "" },
        { name: "Skyspear missile rack", display: "", alias: "" },
        { name: "Swiftstrike burst cannon", display: "", alias: "" },
        { name: "Swiftstrike railgun", display: "", alias: "" },
        { name: "Transport Bay", display: "", alias: "" }
      ]
    },
    {
      name: "Vespid Stingwings",
      points: 65,
      weapons: [
        { name: "Neutron blaster", display: "", alias: "" },
        { name: "Stingwing claws", display: "", alias: "" }
      ]
    }
  ]
}